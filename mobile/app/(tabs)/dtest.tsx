...
function tileScreenPos(gx: number, gy: number) {
  // 45-degree clockwise rotation around board center (DTest only)
  const cx = (BS - 1) / 2;
  const cy = (BS - 1) / 2;
  const dx = gx - cx;
  const dy = gy - cy;
  const angle = -Math.PI / 4; // 45deg clockwise
  const rx = dx * Math.cos(angle) - dy * Math.sin(angle) + cx;
  const ry = dx * Math.sin(angle) + dy * Math.cos(angle) + cy;
  return {
    sx: rx * TILE + TILE / 2,
    sy: ry * TILE * TILT_COS + TILE * TILT_COS / 2,
  };
}
          ...
          <View style={{
            ...
            {Array.from({ length: BS }).map((_, gy) =>
              Array.from({ length: BS }).map((_, gx) => {
                ...
                const __dx = gx - (BS - 1) / 2;
                const __dy = gy - (BS - 1) / 2;
                const __ang = -Math.PI / 4;
                const __rx = __dx * Math.cos(__ang) - __dy * Math.sin(__ang) + (BS - 1) / 2;
                const __ry = __dx * Math.sin(__ang) + __dy * Math.cos(__ang) + (BS - 1) / 2;
                return (
                  <TouchableOpacity
                    key={tileKey}
